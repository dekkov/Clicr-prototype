        ...
        <div className="space-y-6 animate-fade-in">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">Manage who has access to this venue.</p>
                </div>
                <button
            ...
            {isInviting && (
                <div className="bg-card border border-border p-4 rounded-xl flex flex-col md:flex-row gap-4 items-end animate-in fade-in slide-in-from-top-2">
                    <div className="w-full md:w-1/3">
                        <label className="text-xs text-muted-foreground mb-1 block">Name</label>
                        <input
                            type="text"
                            className="w-full bg-muted border border-border rounded-lg px-3 py-2 text-sm text-foreground focus:ring-2 focus:ring-primary outline-none"
            ...
            <div className="grid gap-4">
                {venueUsers.length === 0 ? (
                    ...
                    venueUsers.map(user => (
                        <div key={user.id} className="flex items-center justify-between p-4 bg-card border border-border rounded-xl hover:border-border transition-colors group">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <div className="font-medium text-foreground flex items-center gap-2">
                                        {user.name}
                                        {user.role === 'OWNER' && <span className="text-[10px] bg-amber-500/10 text-amber-500 px-1.5 py-0.5 rounded border border-amber-200 dark:border-amber-500/20">OWNER</span>}
                                        {user.role === 'ADMIN' && <span className="text-[10px] bg-purple-500/10 text-purple-500 px-1.5 py-0.5 rounded border border-purple-200 dark:border-purple-500/20">ADMIN</span>}
                                    </div>
                            ...
                            <div className="flex items-center gap-6">
                                ...
                                {user.role !== 'OWNER' && (
                                    <button
                                        className="p-2 text-muted-foreground hover:text-red-400 hover:bg-red-400/10 rounded-lg transition-colors opacity-0 group-hover:opacity-100"
