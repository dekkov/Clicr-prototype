...
export default async function SignupPage({ searchParams }: Props) {
    ...
    return (
        <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6 relative overflow-hidden">
            <Link href="/" className="absolute top-8 left-8 text-foreground/50 hover:text-foreground flex items-center gap-2 transition-colors font-bold text-sm z-10">
            ...
            <div className="w-full max-w-md bg-card border border-border rounded-3xl p-8 shadow-2xl backdrop-blur-xl">
                <div className="text-center mb-8">
                    ...
                    <h1 className="text-2xl font-bold tracking-tight mb-2 text-foreground">Start your 30-day free trial</h1>
                    <p className="text-foreground/60 text-sm">Create your Clicr account in under 2 minutes.</p>
                </div>
                ...
                <form action={signup} className="space-y-4">
                    <div>
                        ...
                        <input name="email" type="email" required placeholder="you@company.com"
                            className="w-full bg-input border border-border rounded-xl py-3 px-4 text-foreground placeholder-foreground/40 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all" />
                    </div>
                    <div>
                        <label className="block text-xs font-bold text-foreground/50 uppercase tracking-widest mb-2">Password</label>
                        <PasswordInput name="password" required minLength={8} placeholder="8+ characters"
                            className="w-full bg-input border border-border rounded-xl py-3 px-4 pr-10 text-foreground placeholder-foreground/40 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all" />
                    </div>
                    <div>
